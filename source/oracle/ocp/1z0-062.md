...
1. 恢复数据库到控制文件时间点`recover database using backup controlfile;`

2. 恢复数据库到数据文件时间点`recover database;`
